  ...
  def train(train_queue, model, criterion, optimizer):
    ...
    for step, (input, target) in enumerate(train_queue):
      ...
      if not args.cls:
        ...
        logits, logits_aux = model(input[:batch_size // 2])
        loss = torch.mean(criterion(logits, targets_))
        loss += loss.item()

        with torch.no_grad():
          outputs_cls, _ = model(input[batch_size // 2:])
        cls_loss = kdloss(logits, outputs_cls.detach())
